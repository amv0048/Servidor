...
<head>
    ...
    $columna = $_GET["order_by"] ?? "id_pelicula"; // ?? asignacion por defecto
...
<body>
    ...
    if($_SERVER["REQUEST_METHOD"] == "POST"){
        $query = "DELETE FROM peliculas where id_pelicula = '{$_POST["id_pelicula"]}'";
        if ($_conexion->query($query))
            echo '<div class="alert alert-success">Se ha borrado la pelicula con el id: {$_POST["id_pelicula"]}</div>';
        else echo '<div class="alert alert-danger"> NO se ha borrado la pelicula con el id: {$_POST["id_pelicula"]}</div>';
